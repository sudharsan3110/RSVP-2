...
describe('DELETE /v1/cohosts/events/:eventId/:cohostUserId', () => {
  ...
  it('should successfully remove a cohost', async () => {
    ...
    vi.spyOn(CohostRepository, 'FindhostOrCohost').mockImplementation(
      async (userId, eventId, roles = []) => {
    ...
    expect(response.body).toHaveProperty('message', 'Cohost removed successfully');
  });
  it('should return 403 if manager tries to remove creator', async () => {
    ...
    vi.spyOn(CohostRepository, 'FindhostOrCohost').mockImplementation(
      async (userId, eventId, roles = [], returnType = false) => {
        if (returnType) {
    ...
    expect(response.body).toHaveProperty(
      'message',
      API_MESSAGES.COHOST.REMOVE.CANNOT_REMOVE_CREATOR
    );
  });
  ...
  it('should return 400 if user tries to remove self', async () => {
    ...
    const response = await request(app).delete(endpoint);
    expect(response.status).toBe(400);
    expect(response.body).toHaveProperty('message', API_MESSAGES.COHOST.REMOVE.CANNOT_REMOVE_SELF);
  });
  it('should return 400 if user being removed is not a cohost', async () => {
    ...
    const response = await request(app).delete(endpoint);
    expect(response.status).toBe(400);
    expect(response.body).toHaveProperty('message', API_MESSAGES.COHOST.REMOVE.FAILED);
  });
